...
class FreeDipolePSF_torch:
    def __init__(self, psf_pars, req_grad=False):
        ...
        self.wavelength = torch.tensor(self.psf_pars['wavelength'], device='cuda', dtype=torch.float64)
        self.objstage0 = torch.tensor(self.psf_pars['initial_obj_stage'], device='cuda', dtype=torch.float64, requires_grad=req_grad)
        self.zemit0 = torch.tensor(self.psf_pars['zemit0'], device='cuda', dtype=torch.float64, requires_grad=req_grad)
        self.pixel_size_x = torch.tensor(self.psf_pars['pixel_size_x'], device='cuda', dtype=torch.float64)
        self.pixel_size_y = torch.tensor(self.psf_pars['pixel_size_y'], device='cuda', dtype=torch.float64)
    ...
    def gen_psf(self):
        ...
        if len(self.otf_rescale):
            ...
            if I_sigmax == 0 or I_sigmay == 0:
                ...
                h = torch.cuda.DoubleTensor(self.otf_gauss2D(shape=[5, 5], Isigmax=I_sigmax, Isigmay=I_sigmay)) \
                                            .reshape([1, 1, 5, 5])
    ...
    def cal_crlb(self):
        ...
        self.shared = np.concatenate((np.ones(self.num_zernike),np.array([0, 0, 0])),axis=0)
        ...
        n = 0
        for i in range(self.num_zernike+3):
            if self.shared[i] == 1:
                shared_map[n, 0] = 1
                shared_map[n, 1] = i+1
                shared_map[n, 2] = 0
