...
def sample_consumer(queue_local, tif_file, share_mem, read_pos, read_index, write_pos, write_index):
    ...
    while 1:
        try:
            data = queue_local.get(timeout=3)
        except queue.Empty:
            break
...
class PsfInfer:
    def __init__(self,infer_par,shm, tif_file,device,net_params,fd = False):
        ...
        self.model.results_array = torch.Tensor(600000,6).to(device)
        self.res = {}
    ...
    def write_csv(self):
        for i in range(len(self.result_name_list)):
            with open(self.result_name_list+'.csv', 'a', newline='') as csvfile:
        ...
        with torch.set_grad_enabled(False):
            with autocast():
                ...
                while 1:
                    ...
                    img = np.frombuffer(buf[cur_pos+20:cur_pos+20+img_size], dtype=np.float16).reshape((-1,1, self.win_size, self.win_size))
